    ...
    <div class="col-md-12">
        ...
        <div class="card card-navy">
            <div class="card-header">
                ...
                <button class="btn btn-primary btn-sm" id="btn-tambah" data-toggle="tooltip" data-placement="top" title="Klik disini untuk menambah data config"><i class="fa fa-plus-circle"></i></button>
            </div>
            <div class="card-body">
                {{-- Table Star --}}
                <div class="table-responsive">
                    <!-- Tabel -->
                    <table class="table table-hover" id="tabel_config">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Config Code</th>
...
@include('config.modal')
@endsection
@section('scripts')
<script>
    $(document).ready(function() {
        $('#tabel_config').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
        ...
        $('#btn-tambah').click(function(e) {
            ...
            $('#modal-judul').html('Tambah Data config');
        ...
        $('body').on('click', '.edit-post', function() {
            ...
            $.get('config/' + data_id + '/edit', function(data) {
                $('#modal-judul').html("Edit Data config");
                $('#tombol-simpan').html("Rubah");
                $('#modal-tambah-edit').modal('show');

                //set value masing-masing id berdasarkan data yg diperoleh dari ajax get request diatas               
                $('#id').val(data.id);
                $('#config_code').val(data.config_code);
                $('#config_order').val(data.config_order);
        ...
        $('body').on('submit', '#form-tambah-edit', function(e) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    var oTable = $('#tabel_config').dataTable();
                    oTable.fnDraw(false);
        ...
        $('#tombol-hapus').click(function() {
            $.ajax({
                ...
                url: "config/" + dataId, //eksekusi ajax ke url ini
                type: 'delete',
                beforeSend: function() {
